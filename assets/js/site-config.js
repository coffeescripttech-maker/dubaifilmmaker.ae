  ...
  function handleNavigationLinks(navConfig) {
    const navItems = {
      works: {
        ...
        selector: 'li[data-slug="works"]'
      },
      about: {
        enabled: navConfig.aboutPage.enabled,
        selector: 'li[data-slug="about"]'
      },
      contact: {
        enabled: navConfig.contactPage.enabled,
        selector: 'li[data-slug="contact"]'
      }
    };

    Object.entries(navItems).forEach(([page, settings]) => {
      // Find the <li> element (not just the link)
      const navItem = document.querySelector(settings.selector);
      ...
      if (navItem) {
        if (!settings.enabled) {
          ...
          // Also disable the link inside for extra safety
          const link = navItem.querySelector('a');
          if (link) {
            disableLink(link);
          }
          
          console.log(`✓ ${page} navigation item hidden (mobile & desktop)`);
        } else {
          // Show the navigation item
          navItem.style.display = '';
          
          // Enable the link inside
          const link = navItem.querySelector('a');
          if (link) {
            enableLink(link);
          }
          
          console.log(`✓ ${page} navigation item visible (mobile & desktop)`);
        }
      }
    });
  }
